...
!function($){
  ...
  $.fn.flyIn = function (el, button, width, height, angle, step, radius_w, radius_h, settings) {
    var d = 0;
    this.stop(true,true);
    this.each(function(index) {
      angle = (settings.angle[0] + (step * index)) * (Math.PI/180); 
      var x = Math.round(width/2 + radius_w * Math.cos(angle) - $(this).find("a").outerWidth()/2),
          y = Math.round(height/2 + radius_h * Math.sin(angle) - $(this).find("a").outerHeight()/2);
  ...
  $.fn.fadeInIcon = function (el, button, width, height, angle, step, radius_w, radius_h, settings) {
    var d = 0;
    this.stop(true,true);
    this.each(function(index) {
      angle = (settings.angle[0] + (step * index)) * (Math.PI/180); 
  ...
  $.fn.fadeOutIcon = function (el, button) {
    ...
    settings = predefineAngle(settings);
	  var radius_w = (el.width() / 2) + 50;
      var radius_h = (el.height() / 2) + 50;
      fields = el.find(".item"),
      container = el,
      ...
      switch (settings.animation) { 
        case 'fade': 
          fields.fadeInIcon(el, button, width, height, angle, step, radius_w, radius_h, settings)
          break; 
          
        case 'fly': 
          fields.flyIn(el, button, width, height, angle, step, radius_w, radius_h, settings)
          break; 
      }
	...
	function predefineSpeed(settings) {
      ...
      switch (settings.animationSpeed) { 
        ...
        case 'medium':
          ...
          break;
        case 'fast':
          settings.animationSpeed = [25,150]
          break;
        case 'instant':
